...
export default function ParentProfile() {
  ...
  const sidebarItems = [
    ...
    { id: 'connection', icon: UserPlus, label: 'Kết nối Chuyên gia' },
  ];
  ...
  const fetchConnections = async () => {
    const { data } = await supabase
      .from('connections')
      .select('*, therapist:profiles!connections_therapist_id_fkey(full_name, clinic_name)')
  ...
  const handleConnect = async () => {
    if (!therapistCode.trim()) {
      toast({ title: "Lỗi", description: "Vui lòng nhập mã chuyên gia", variant: "destructive" });
      return;
    }

    setConnecting(true);
    try {
      // 1. Find therapist by code (Case-insensitive search)
      const { data: therapist, error: searchError } = await supabase
        .from('profiles')
        .select('user_id, full_name')
        .ilike('therapist_code', therapistCode.trim())
        .maybeSingle();

      if (searchError || !therapist) {
        toast({ title: "Không tìm thấy", description: "Mã chuyên gia không hợp lệ", variant: "destructive" });
        return;
      }
      ...
      if (connectError) {
        if (connectError.code === '23505') {
          toast({ title: "Thông báo", description: "Bạn đã gửi yêu cầu kết nối tới chuyên gia này rồi" });
        } else {
          throw connectError;
        }
      } else {
        toast({ title: "Thành công", description: `Đã gửi yêu cầu kết nối tới chuyên gia ${therapist.full_name}` });
        setTherapistCode('');
        fetchConnections();
      }
    } catch (error: any) {
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <main className="flex-1 space-y-8">
          ...
          {activeTab === 'connection' && (
            <section className="bg-white dark:bg-[#1a2e2c] rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden animate-in fade-in slide-in-from-bottom-4">
              <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
                <div>
                  <h2 className="text-lg font-bold text-gray-900 dark:text-white">Kết nối Chuyên gia</h2>
                  <p className="text-sm text-gray-500 mt-1">Kết nối với Therapist để họ có thể theo dõi tiến trình của bé.</p>
                </div>
                <UserPlus size={20} className="text-gray-400" />
              </div>
              <div className="p-6 space-y-8">
                {/* Input Area */}
                <div className="bg-[#E0F2F1]/50 dark:bg-[#00695C]/10 rounded-2xl p-6 border border-[#00695C]/20">
                  <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Nhập mã chuyên gia</h3>
                  <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex-1 relative">
                      <Shield size={18} className="absolute left-3 top-3 text-[#00695C]" />
                      <input
                        type="text"
                        value={therapistCode}
                        onChange={(e) => setTherapistCode(e.target.value.toUpperCase())}
                        placeholder="Ví dụ: DR-A1B2C3"
                        className="w-full pl-10 pr-4 py-2.5 rounded-xl border-0 ring-1 ring-[#00695C]/30 focus:ring-2 focus:ring-[#00695C] text-sm bg-white dark:bg-[#1a2e2c] text-gray-900 dark:text-white font-mono uppercase"
                      />
                    </div>
                    <button
                      onClick={handleConnect}
                      disabled={connecting}
                      className="px-8 py-2.5 bg-[#00695C] text-white font-bold rounded-xl shadow-lg shadow-[#00695C]/20 hover:bg-[#004D40] disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                    >
                      {connecting && <RefreshCw className="animate-spin" size={18} />}
                      Gửi yêu cầu
                    </button>
                  </div>
                  <p className="text-[11px] text-gray-500 mt-3 italic">
                    * Yêu cầu chuyên gia cung cấp mã code của họ trong phần "Cài đặt tài khoản".
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  <h3 className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <Check size={18} className="text-[#00695C]" />
                    Chuyên gia đã kết nối ({connections.length})
                  </h3>

                  {connections.length === 0 ? (
                    <div className="text-center py-12 bg-gray-50 dark:bg-[#152523] rounded-2xl border border-dashed border-gray-200 dark:border-gray-800">
                      <MessageSquare size={40} className="mx-auto text-gray-300 mb-3" />
                      <p className="text-sm text-gray-400">Chưa có kết nối nào. Hãy nhập mã chuyên gia để bắt đầu.</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {connections.map((conn) => (
                        <div key={conn.id} className="p-4 rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-[#1a2e2c] hover:shadow-md transition-all">
                          <div className="flex items-center gap-3 mb-3">
                            <div className="w-10 h-10 rounded-full bg-[#00695C]/10 text-[#00695C] flex items-center justify-center font-bold">
                              {(conn.therapist?.full_name || 'T')[0]}
                            </div>
                            <div>
                              <h4 className="text-sm font-bold text-gray-900 dark:text-white">{conn.therapist?.full_name}</h4>
                              <p className="text-[11px] text-gray-500">{conn.therapist?.clinic_name || 'Chuyên gia phục hồi'}</p>
                            </div>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full uppercase ${conn.status === 'accepted'
                              ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
                              : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'
                              }`}>
                              {conn.status === 'accepted' ? 'Đã kết nối' : 'Đang chờ duyệt'}
                            </span>
                            <button className="text-[11px] text-red-500 hover:text-red-600 font-medium">Hủy kết nối</button>
                          </div>
                        </div>
                      ))}
                    </div>
