...
const RuleSwitcher = () => {
  ...
  // Save telemetry data
  const saveTelemetry = async (sid: string, telemetry: TrialData[]) => {
  ...
  // Complete session with advanced metrics
  const completeSession = async (sid: string) => {
    ...
    const avgSwitchTime = switchTrials.length > 0
      ? switchTrials.reduce((sum, t) => sum + t.reactionTime, 0) / switchTrials.length
      : 0;
    const avgRepeatTime = repeatTrials.length > 0
      ? repeatTrials.reduce((sum, t) => sum + t.reactionTime, 0) / repeatTrials.length
      : 0;
    ...
    const advancedMetrics = {
      switch_cost_ms: Math.round(switchCostMs),
      inhibition_accuracy: Math.round(inhibitionAccuracy),
  ...
  return (
    <GameLayout className={cn(flashWhite && "bg-white")}>
      {/* Header - only show during gameplay */}
      {phase === "playing" && (
        <GameHeader
          title="Stroop Chaos"
          subtitle={RULE_LABELS[currentRule].instruction}
          ...
          <AnimatePresence mode="wait">
            {phase === "intro" && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -20 }}
              >
                <Card className="border-orange-200/50 dark:border-orange-800/50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-xl">
                  <CardHeader className="text-center pb-2">
                    <motion.div
                      className="inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-br from-orange-500 to-rose-500 mx-auto mb-4 shadow-lg shadow-orange-500/30"
                      animate={{ rotate: [0, 5, -5, 0], scale: [1, 1.05, 1] }}
                      transition={{ repeat: Infinity, duration: 2 }}
                    >
                      <Brain className="w-10 h-10 text-white" />
                    </motion.div>
                    <CardTitle className="text-2xl text-slate-800 dark:text-white">Stroop Chaos</CardTitle>
