...
import { Radio, Terminal, Hash, ChevronRight, AlertTriangle, Trash2, CheckCircle2, RotateCcw } from "lucide-react";
...
const DispatcherAssessment = () => {
    ...
    const verifyCode = () => {
        ...
        if (inputCode === currentCode) {
            ...
            const finishTimeout = setTimeout(() => {
                ...
                // Simplified: Just 1 chance per level for now to measure pure Peak Performance?
    ...
    const finishGame = async () => {
        ...
        await supabase.from("game_sessions").update({
            ...
            final_score: maxSpan * 10, // Simple score
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-amber-500 font-mono flex flex-col items-center justify-center p-4 overflow-hidden relative selection:bg-amber-500 selection:text-black">
            {/* CRT Scanline Overlay */}
            <div className="pointer-events-none fixed inset-0 z-50 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.25)_50%),linear-gradient(90deg,rgba(255,0,0,0.06),rgba(0,255,0,0.02),rgba(0,0,255,0.06))] z-10 bg-[length:100%_2px,3px_100%] opacity-20" />
            ...
            <div className="w-full max-w-lg relative z-20">
                {phase === "intro" ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="border-2 border-amber-900 bg-black/80 p-6 shadow-[0_0_20px_rgba(245,158,11,0.1)]">
                        ...
                        <Button onClick={handleStart} className="w-full h-12 bg-amber-600 hover:bg-amber-500 text-black font-bold font-mono tracking-wider rounded-none">
                            INITIATE SEQUENCE
                        </Button>
                    </motion.div>
                ) : phase === "completed" ? (
